...
class DTLSClientSocketImpl::SSLContext {
 public:
  ...
  DTLSClientSocketImpl* GetClientSocketFromSSL(const SSL* ssl) {
    DCHECK(ssl);
    DTLSClientSocketImpl* socket = static_cast<DTLSClientSocketImpl*>(
        SSL_get_ex_data(ssl, ssl_socket_data_index_));
    DCHECK(socket);
    return socket;
  }

  bool SetClientSocketForSSL(SSL* ssl, DTLSClientSocketImpl* socket) {
 ...
 private:
  ...
  SSLContext() {
    ...
    // Added DTLS mode
    ssl_ctx_.reset(SSL_CTX_new(DTLS_method()));
  ...
  static int ClientCertRequestCallback(SSL* ssl, void* arg) {
    DTLSClientSocketImpl* socket = GetInstance()->GetClientSocketFromSSL(ssl);
    DCHECK(socket);
  ...
  // DTLSClientSocketImpl object from an SSL instance.
...
const SSL_PRIVATE_KEY_METHOD
    DTLSClientSocketImpl::SSLContext::kPrivateKeyMethod = {
        &DTLSClientSocketImpl::SSLContext::PrivateKeySignCallback,
        nullptr /* decrypt */,
        &DTLSClientSocketImpl::SSLContext::PrivateKeyCompleteCallback,
};
...
void DTLSClientSocketImpl::SetSSLKeyLogger(
    std::unique_ptr<SSLKeyLogger> logger) {
  SSLContext::GetInstance()->SetSSLKeyLogger(std::move(logger));
}

int DTLSClientSocketImpl::ExportKeyingMaterial(const base::StringPiece& label,
                                              bool has_context,
...
int DTLSClientSocketImpl::Connect(CompletionOnceCallback callback) {
...
void DTLSClientSocketImpl::Disconnect() {
  disconnected_ = true;
...
// at once.
int DTLSClientSocketImpl::ConfirmHandshake(CompletionOnceCallback callback) {
...
bool DTLSClientSocketImpl::IsConnected() const {
...
bool DTLSClientSocketImpl::IsConnectedAndIdle() const {
  // If the handshake has not yet completed or the socket has been explicitly
  // disconnected.
  if (!completed_connect_ || disconnected_)
    return false;
...
int DTLSClientSocketImpl::GetPeerAddress(IPEndPoint* addressList) const {
  return stream_socket_->GetPeerAddress(addressList);
}

int DTLSClientSocketImpl::GetLocalAddress(IPEndPoint* addressList) const {
  return stream_socket_->GetLocalAddress(addressList);
}

const NetLogWithSource& DTLSClientSocketImpl::NetLog() const {
  return net_log_;
}

bool DTLSClientSocketImpl::WasEverUsed() const {
  return was_ever_used_;
}

bool DTLSClientSocketImpl::WasAlpnNegotiated() const {
  return negotiated_protocol_ != kProtoUnknown;
}

NextProto DTLSClientSocketImpl::GetNegotiatedProtocol() const {
  return negotiated_protocol_;
}

bool DTLSClientSocketImpl::GetSSLInfo(SSLInfo* ssl_info) {
  ssl_info->Reset();
  if (!server_cert_)
    return false;
...
void DTLSClientSocketImpl::GetConnectionAttempts(ConnectionAttempts* out) const {
  out->clear();
}

int64_t DTLSClientSocketImpl::GetTotalReceivedBytes() const {
  return stream_socket_->GetTotalReceivedBytes();
}

void DTLSClientSocketImpl::DumpMemoryStats(SocketMemoryStats* stats) const {
...
void DTLSClientSocketImpl::GetSSLCertRequestInfo(
...
void DTLSClientSocketImpl::ApplySocketTag(const SocketTag& tag) {
  return stream_socket_->ApplySocketTag(tag);
}

int DTLSClientSocketImpl::Read(IOBuffer* buf,
                              int buf_len,
...
int DTLSClientSocketImpl::ReadIfReady(IOBuffer* buf,
                                     int buf_len,
                                     CompletionOnceCallback callback) {
...
int DTLSClientSocketImpl::CancelReadIfReady() {
...
int DTLSClientSocketImpl::Write(
    IOBuffer* buf,
    int buf_len,
    CompletionOnceCallback callback,
...
int DTLSClientSocketImpl::SetReceiveBufferSize(int32_t size) {
  return stream_socket_->SetReceiveBufferSize(size);
}

int DTLSClientSocketImpl::SetSendBufferSize(int32_t size) {
  return stream_socket_->SetSendBufferSize(size);
}

void DTLSClientSocketImpl::OnReadReady() {
  // During a renegotiation, either Read or Write calls may be blocked on a
  // transport read.
  RetryAllOperations();
}

void DTLSClientSocketImpl::OnWriteReady() {
  // During a renegotiation, either Read or Write calls may be blocked on a
  // transport read.
  RetryAllOperations();
}
...
void DTLSClientSocketImpl::DoReadCallback(int rv) {
...
void DTLSClientSocketImpl::DoWriteCallback(int rv) {
...
int DTLSClientSocketImpl::DoHandshake() {
  crypto::OpenSSLErrStackTracer err_tracer(FROM_HERE);
...
int DTLSClientSocketImpl::DoHandshakeComplete(int result) {
  if (result < 0)
    return result;
...
ssl_verify_result_t DTLSClientSocketImpl::VerifyCertCallback(
    SSL* ssl,
    uint8_t* out_alert) {
  DTLSClientSocketImpl* socket =
...
// OpenSSL error queue.
ssl_verify_result_t DTLSClientSocketImpl::VerifyCert() {
  ...
  cert_verification_result_ = cert_verifier_->Verify(
      ...
      base::BindOnce(&DTLSClientSocketImpl::OnVerifyComplete,
...
void DTLSClientSocketImpl::OnVerifyComplete(int result) {
...
void DTLSClientSocketImpl::DoConnectCallback(int rv) {
  if (!user_connect_callback_.is_null()) {
    std::move(user_connect_callback_).Run(rv > OK ? OK : rv);
  }
}

void DTLSClientSocketImpl::OnHandshakeIOComplete(int result) {
...
int DTLSClientSocketImpl::DoHandshakeLoop(int last_io_result) {
  TRACE_EVENT0(NetTracingCategory(), "DTLSClientSocketImpl::DoHandshakeLoop");
...
int DTLSClientSocketImpl::DoPayloadWrite() {
  crypto::OpenSSLErrStackTracer err_tracer(FROM_HERE);
...
void DTLSClientSocketImpl::RetryAllOperations() {
  ...
  base::WeakPtr<DTLSClientSocketImpl> guard(weak_factory_.GetWeakPtr());
...
int DTLSClientSocketImpl::VerifyCT() {
...
int DTLSClientSocketImpl::ClientCertRequestCallback(SSL* ssl) {
  DCHECK(ssl == ssl_.get());
...
int DTLSClientSocketImpl::NewSessionCallback(SSL_SESSION* session) {
  if (!IsCachingEnabled())
    return 0;
...
void DTLSClientSocketImpl::AddCTInfoToSSLInfo(SSLInfo* ssl_info) const {
  ssl_info->UpdateCertificateTransparencyInfo(ct_verify_result_);
}

std::string DTLSClientSocketImpl::GetSessionCacheKey() const {
...
bool DTLSClientSocketImpl::IsRenegotiationAllowed() const {
...
bool DTLSClientSocketImpl::IsCachingEnabled() const {
  return ssl_client_session_cache_ != nullptr;
}

ssl_private_key_result_t DTLSClientSocketImpl::PrivateKeySignCallback(
    uint8_t* out,
    size_t* out_len,
    size_t max_out,
    uint16_t algorithm,
  ...
  ssl_config_.client_private_key->Sign(
      ...
      base::BindOnce(&DTLSClientSocketImpl::OnPrivateKeyComplete,
                     weak_factory_.GetWeakPtr()));
  return ssl_private_key_retry;
}

ssl_private_key_result_t DTLSClientSocketImpl::PrivateKeyCompleteCallback(
    uint8_t* out,
    size_t* out_len,
    size_t max_out) {
  DCHECK_NE(kSSLClientSocketNoPendingResult, signature_result_);
...
void DTLSClientSocketImpl::OnPrivateKeyComplete(
    Error error,
...
void DTLSClientSocketImpl::InfoCallback(int type, int value) {
...
void DTLSClientSocketImpl::MessageCallback(int is_write,
                                          int content_type,
                                          const void* buf,
                                          size_t len) {
  switch (content_type) {
...
void DTLSClientSocketImpl::LogConnectEndEvent(int rv) {
  if (rv != OK) {
...
void DTLSClientSocketImpl::RecordNegotiatedProtocol() const {
  UMA_HISTOGRAM_ENUMERATION("Net.SSLNegotiatedAlpnProtocol",
                            negotiated_protocol_, kProtoLast + 1);
}

int DTLSClientSocketImpl::MapLastOpenSSLError(
    int ssl_error,
